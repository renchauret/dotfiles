...
7. Report your testing results in the Testing Details → Testing notes field in the associated JIRA ticket
   ...
   3. If there are already Testing notes, this was likely from a previous backend-preprod-tester pass; you should overwrite them
...
temporarily remove this tag and adhoc the commit with it missing, allowing you to bypass the required auth.
Immediately push a follow-up commit with the tag added back.
...
If you weren't prompted by ren directly, ask your parent agent for the `otp` code and tell them to relay the request up the agent chain until it gets ren.
